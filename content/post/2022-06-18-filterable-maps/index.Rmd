...
To build the map, we'll need the following collection of `R` packages:
...
Each row of the filters are derived from calls to the `bscols` function in the `crosstalk` package, which enable us to align objects across columns within the row. From there, we can add dropdown selectors with `filter_select` or slider selectors with `filter_slider`. Here's a breakdown of each of these rows:
