...
We know that there are infinitely many possibilities for $p$. Let's first go through this assuming the globe is that of a 4-sided die, such that the only possibilities are $p \in (0,.25,.50,.75,1)$. For each possible value of $p$, what is number of ways we could have observed our data? (thinking of the generative process, starting with $N$ and $p$).
...
# Number of ways to observe sample (for each p)
...
What is key to note about the posterior probabilities is that they are relative to the total across all values of $p$. So we could approach this differently and get the same result:

1. We could think of each observed value as an (unfair) coin flip (according to the value of $p$) and calculate the likelihood of the sequence of flips:
...
# Compute posterior
likelihood / sum(likelihood) # Same as before
```

2. We could think of it as finding the likelihood of observing the total number of water spins (which is the same as the prior example, except we're accounting for all of the combinations as well):
...
$$P(W|p) = \frac{N!}{W!(N-W)!}p^W(1-p)^{(N-W)}$$
...
\end{equation}
$$
We have a _uniform_ prior on the parameter $p$, so we can factor out that term:
...
\end{equation}
$$
As you can see, the combination term also factors out of each term, so when computing the posterior probability, it doesn't matter if we use the counts, base likelihood, or the probability distribution.
