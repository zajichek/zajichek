...
As expected, the p-value of `r round(equitest(compoisson_model1)$pvalue, 4)` does not indicate that over or under dispersion was detected.
...
So how well does the likelihood ratio test [described above](#compoissonlrt) detect over or under dispersion in the data-generating process? We can use simulation! To keep it simple, we will base it off of intercept-only regression models with no covariates, in effect assessing power at varying values of $\lambda$. Presumably, power may depend on the complexity of the model, so coverage might vary as terms are added. Here is the setup for the simulation using the previous [parameter grid](#centralitysimulation):
...
result <-
  ...
  # Cross-join to add sample size to the parameter grid
  inner_join(
    y = 
      tibble(
  ...
  map_df(
    function(x) {
      ...
      # 1. Generate sample for the specified parameter set
      y_i <- rcmp(n = x$N, lambda = x$lambda, nu = x$nu)
      
      # 2. Fit the COM-Poisson model
      model <- glm.cmp(y_i ~ 1)
      
      # 3. Compute the p-value
      pvalue <- equitest(model)$pvalue
