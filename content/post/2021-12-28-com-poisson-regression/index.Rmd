...
Let's jump into a quick example to see how a model is fit with the `glm.cmp` function. We'll use the `ed_volumes` dataset from [previous section](#reviewofpoisson) to fit a COM-Poisson model by regressing the day of week and AM/PM indicators on the ED patient volume.

```{r}
compoisson_model1 <-
  glm.cmp(
    formula.lambda = Volume ~ Weekend + PM,
    data = 
      ed_volumes %>% 
      mutate(
        Weekend = weekdays(Date) %in% c("Saturday", "Sunday"),
...
If we take a quick look at the p-values, we see some familiar results. Namely, the day of week indicator appears to be associated with ED volumes, and the AM/PM indicator does not. Looking closer, there is an additional parameter estimated in the output that we did not see in the standard Poisson model--this is where the dispersion comes in. In addition to estimating the effects of each covariate on the $\lambda$ parameter, it also estimated the dispersion parameter value [discussed above](#compoissondistribution). We can look at this output to determine if there is evidence of over/under dispersion. The package also provides the `equitest` function to carry out the _likelihood ratio test_ for the following hypotheses:

$$H_0: \nu = 1 \hskip.25in H_A: \nu \neq 1$$
It tests whether there is evidence that the data is _not_ equidispersed. It does not indicate a direction, so it is up to us to look at the estimated parameter value to know if over or under dispersion was detected. We get the following output when performing this test on our model:

```{r}
equitest(compoisson_model1)
```

The p-value of `r round(equitest(compoisson_model1)$pvalue, 4)` does not indicate that over or under dispersion was detected. This is of course expected here, because we generated this dataset from a standard Poisson model, which we know means that $\nu=1$.

* Notice the `formula.lambda` argument
