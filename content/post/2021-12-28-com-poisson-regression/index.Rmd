...
Now that we have a foundation for standard Poisson regression, we can dig into COM-Poisson! We will use the [`COMPoissonReg`](https://github.com/lotze/COMPoissonReg) package, which can be installed from CRAN with `install.packages("COMPoissonReg")`.
...
1. The $\lambda$ parameter, which _is_ the mean when $\nu=1$ (standard Poisson), is the mean of the

$$E[Y^\nu] = \lambda$$
2. The mean and variance can be approximated when $\nu \leq 1$ (overdispersed) or $\lambda > 10^\nu$ (large counts):

$$E[Y] \approx \lambda^{1/\nu} - \frac{\nu-1}{2\nu}$$
$$Var[Y] \approx \frac{\lambda^{1/\nu}}{\nu}$$
This is a bit disappointing because we have methods for those cases. The seemingly more appealing case of this method is when we have underdispersed data with small counts.

### Simulated Assessment

The properties above give us some information, but it is hard to make sense of exactly how these parameters will govern the actual data we might observe from a COM-Poisson distribution. Additionally, the approximations above hold in cases where we may tend to use alternative methods (e.g., negative-binomial models). The more interesting case, and the focus here, is how these methods can help us for underdispersed data (with smaller counts). To give us a bit more insight into this behavior, lets randomly generate a bunch of data for different parameter combinations and see what we find.
...
params %>%
  ...
  map_df(
    ~
      tibble(
        value = rcmp(n = n, lambda = .x$lambda, nu = .x$nu),
        lambda = .x$lambda,
        nu = .x$nu
      )
  )
...
* Plots of actual observed data
* Simulation of sample means/variances and it's relation to nu and lambda
