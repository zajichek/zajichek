...
Now that we have a foundation for standard Poisson regression, we can dig into COM-Poisson!
...
Plugging that result back into the full PMF, it turns out to be _exactly_ the same as a Poisson distribution. This tells us that **_the Poisson distribution is a special case of the COM-Poisson_**.

### Dispersion Parameter

An important feature of the COM-Poisson PDF is the parameter $\nu$, which determines its _dispersion_ (i.e., how variable it is relative to its mean). The following table describes the properties when it is above, below, and equal to 1:

Parameter|Dispersion|Moments
------|-----------|----------
$\nu=1$|Equidispersed|$E[Y] = Var[Y]$
$\nu<1$|Overdispersed|$E[Y] < Var[Y]$
$\nu>1$|Underdispersed|$E[Y] > Var[Y]$

### Mean and Variance

There is [no closed form solution](https://faculty.georgetown.edu/kfs7/MY%20PUBLICATIONS/COMPoissonModelForCountDataWithDiscussion.pdf) for the mean and variance of a COM-Poisson random variable. However, there are a couple related properties:
