...
The model structure for COM-Poisson is very similar to the [standard Poisson model](#modelformulation). In fact, we can just copy and paste the model form above down here:

$$log(\lambda_i) = \beta_0 + \beta_1x_{i1} + \beta_2x_{i2}+...+\beta_px_{ip} = X_i\beta$$
Just like in standard model, we are modeling the (log) $\lambda$ parameter as a linear combination of some covariates. The only difference here is that this $\lambda$ parameter no longer represents the adjusted mean. So, the steps for estimating the parameters are also the same:
