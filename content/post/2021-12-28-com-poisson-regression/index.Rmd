...
ed_volumes <-
  tibble(
    ...
    Volume = 
      rpois(
        n = length(Date), 
        lambda = ifelse(weekdays(Date) %in% c("Saturday", "Sunday"), ed_true_mean - 5, ed_true_mean + 5)
      )
  )
...
There were `r round(mean(ed_volumes$Volume), 1)` patients per day on average, and the volume ranged from `r min(range(ed_volumes$Volume))` to `r max(range(ed_volumes$Volume))`. What can we do with this information? Can we make estimates about the likelihood that we see more than, say, 50 patients in a day? First, let's revisit the Poisson distribution:

## The Poisson distribution

The Poisson distribution is used to model a count of the occurrences of some event __Y__, generally in a fixed amount of time thus, it can be interpreted as a rate. More formally, if the probability mass function (PMF) for a non-negative, integer-valued __Y__ is:

$$P(Y = y|\lambda) = \frac{e^{-\lambda}\lambda^y}{y!}$$
then __Y__ is distributed as a Poisson random variable, which implies the following property:
...
### 1. Create simulated dataset for ED example
...
# Build the dataset; differening means by weekend/weekday
ed_volumes <-
  tibble(
    Date = Sys.Date() - seq(1, 365, 1),
    Volume = 
