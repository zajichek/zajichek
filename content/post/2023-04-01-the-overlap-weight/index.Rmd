...
* [Setting up the potential outcomes](#potentialoutcomes)
  ...
  + [Generate the observed outcome](#observedoutcome)
...
$$log(\frac{p_i^A}{1 - p_i^A}) = 0.405*age_z - 0.223*male - 0.693*income_z - 0.4$$
...
$$f(t) = \frac{\alpha}{\sigma}\left(\frac{x}{\sigma}\right)^{\alpha-1}e^{-\left(\frac{t}{\sigma}\right)^\alpha}$$
...
where $T_i^A$ and $T_i^B$ are the event times under treatments _A_ and _B_, respectively. That is, the event times for each patient under each treatment follow a [Weibull]() distribution with a common _shape_ parameter (in this case, across both treatments), but a _scale_ parameter that depends on the patient's specific characteristics and differs by treatment, which captures the treatment effect.

### Relating 

* Using Weibull model for outcome 
* Assume the linear predictors; determines treatment effect
* Parameterization (SAS code vs. R); write out PDF linking linear predictor
* How we interpret the effect mathematically

## Generate the observed outcome {#observedoutcome}

* Outcome observed depends on what treatment was received
* Also depends on whether or not the patient was followed long enough to see the outcome happen (so may be censored)
