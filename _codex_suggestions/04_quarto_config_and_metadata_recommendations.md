...
## Recommendation 1: Add Explicit Sitemap Configuration

### Purpose
Sitemaps help search engines discover and refresh pages. Quarto appears to be generating `_site/sitemap.xml`, but making sitemap behavior explicit in source config improves maintainability.

### Current issue
`_site/sitemap.xml` exists in rendered output, and `_quarto.yml` has `site-url: https://www.zajichekstats.com/`. However, `_quarto.yml` does not explicitly show sitemap configuration.

### Suggested change
Add explicit sitemap configuration in `_quarto.yml` after confirming the current Quarto version supports the syntax you intend to use.
...
2. Check Quarto documentation for the exact sitemap option supported by that version.
3. Add the smallest explicit sitemap setting.
4. Render the site.
5. Confirm `_site/sitemap.xml` is still generated.

### Risk / possible site issues
If the option name differs by Quarto version, an invalid YAML key may be ignored or produce a warning. Verify with a render.

### How to verify
Run `quarto render` and confirm `_site/sitemap.xml` exists and includes the expected public pages.

## Recommendation 2: Strengthen Site-Wide Metadata Defaults
...
## Recommendation 3: Add Twitter/X Card Metadata If Social Sharing Matters
...
## Recommendation 4: Use A Shared Blog Listing Configuration Where Possible
...
## Recommendation 5: Add A Footer Structure With Useful Links
...
## Recommendation 6: Consider A Lightweight Robots File Source If Deployment Allows
...
### Suggested change
If Quarto/deployment does not already generate the desired robots file, consider adding a simple source `robots.txt`.
